...
import {
  translateCall,
  internalCoherenceCall,
  missingRequiredDataCall,
  checkAvailableProductsCall,
  inappropriateRequestsCall,
  precedenceLookupCall,
  applyStaticCategoryRulesCall,
  approvalTierCall,
  purelyEligibleSuppliersCall,
  restrictedSuppliersCall,
  geographicalRulesCall,
  evaluatePreferredSupplierCall,
  applyDynamicCategoryRulesCall,
  pricingCalculationCall,
  reevaluateTierCall,
  scoringAndRankingCall,
  finalCheckCall,
} from "@/lib/api-calls";
...
function hasBlocking(data: RequestData): boolean {
  return Object.values(data.stages).some(
    (stage) =>
      stage.issues.some((i) => i.blocking) ||
      stage.escalations.some((e) => e.blocking),
  );
}
...
export async function core_agent(
  ...
  let currentData =
    options?.resumeFrom?.data ?? createRequestData(uploadedJson);
  ...
  async function runStage(
    nodeName: string,
    makeCall: () => Promise<RequestDataPatch>,
  ): Promise<void> {
  ...
  function abort(after: string) {
    console.log(
      `[core_agent] pipeline aborted after ${after} — blocking issue or escalation detected`,
    );
  }
  ...
  await Promise.all([
    ...
    (async () => {
      ...
      await runStage("missing_required_data", () =>
        missingRequiredDataCall(interp),
      );
      if (shouldAbort()) return;
      await runStage("check_available_products", () =>
        checkAvailableProductsCall(interp),
      );
    })(),
  ]);
  ...
  await runStage("inappropriate_requests", () =>
    inappropriateRequestsCall(interp),
  );
  if (shouldAbort()) {
    abort("inappropriate_requests");
    return currentData;
  }
  ...
  await Promise.all([
    ...
    runStage("apply_category_rules", () =>
      applyStaticCategoryRulesCall(interp),
    ),
  ]);
  ...
  await runStage("purely_eligible_suppliers", () =>
    purelyEligibleSuppliersCall(currentData.request_interpretation),
  );
  if (shouldAbort()) {
    abort("purely_eligible_suppliers");
    return currentData;
  }
  ...
  await Promise.all([
    (async () => {
      await runStage("restricted_suppliers", () =>
        restrictedSuppliersCall(currentData),
      );
      if (shouldAbort()) return;
      await runStage("geographical_rules", () =>
        geographicalRulesCall(currentData),
      );
    })(),
    runStage("evaluate_preferred_supplier", () =>
      evaluatePreferredSupplierCall(currentData),
    ),
  ]);
  ...
  await runStage("apply_dynamic_category_rules", () =>
    applyDynamicCategoryRulesCall(currentData),
  );
  if (shouldAbort()) {
    abort("apply_dynamic_category_rules");
    return currentData;
  }

  // ── pricingCalculation ────────────────────────────────────────────────────
  await runStage("pricing_calculation", () =>
    pricingCalculationCall(currentData),
  );
  if (shouldAbort()) {
    abort("pricing_calculation");
    return currentData;
  }

  // ── reevaluateTier ────────────────────────────────────────────────────────
  await runStage("reevaluate_tier_from_quote", () =>
    reevaluateTierCall(currentData),
  );
  if (shouldAbort()) {
    abort("reevaluate_tier_from_quote");
    return currentData;
  }

  // ── scoringAndRanking ─────────────────────────────────────────────────────
  await runStage("scoring_and_ranking", () =>
    scoringAndRankingCall(currentData),
  );
  if (shouldAbort()) {
    abort("scoring_and_ranking");
    return currentData;
  }
