...
export const TERMINAL_STATUSES = new Set<PipelineNodeStatus>([
  "done",
  "warning",
  "escalation",
]);
...
export function deriveStatus(
  nodeName: string,
  patch: RequestDataPatch,
): PipelineNodeStatus {
  const stageResult = patch.stages?.[nodeName as StageId];
  const hasBlocking =
    (stageResult?.escalations?.length ?? 0) > 0 ||
    (stageResult?.issues?.some((i) => i.blocking) ?? false);
  const hasWarning = (stageResult?.issues?.length ?? 0) > 0;

  // console.log("escalation:", hasBlocking)
